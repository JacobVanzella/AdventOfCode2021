...
int main(int argc, char *argv[])
{
    ...
    for (auto line : lines)
    {
        ...
        {
            lineLen = abs(line.first.first - line.second.first);
            forward = line.second.first - line.first.first > 0;  // Forward == true
            upward = line.second.second - line.first.second < 0; // Upward == true
            
            if (forward && upward)
                for (int i = 0; i <= lineLen; i++)
                    map[line.first.second - i][line.first.first + i]++;
            else if (!forward && upward)
                for (int i = 0; i <= lineLen; i++)
                    map[line.first.second - i][line.first.first - i]++;
            else if (forward && !upward)
                for (int i = 0; i <= lineLen; i++)
