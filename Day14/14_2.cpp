...
void updatePolymere(
    ...
    std::map<std::string, int64_t> &links,
    std::map<std::string, int64_t> &tmpLinks,
...
{
    int64_t steps = 40, min = INT64_MAX, max = INT64_MIN;
    std::string pair, put, pTemplate;
    std::fstream input("input.txt");
    std::map<char, int64_t> occurences;
    std::map<std::string, int64_t> links, tmpLinks;
    std::map<std::string, std::string> rules;

    // Get initial template pair
    input >> pTemplate;
    input.ignore(1, ' ');
    for (int64_t i = 0; i < pTemplate.size() - 1; i++)
    {
